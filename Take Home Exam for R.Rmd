...
但是在存在异方差的时候，也就是模型2中，只有怀特标准误的方法比较好的发挥了作业，也就是犯第二类错误的概率是0.05。
...
We can find the optimal models are not the same here. This is beacuse AIC and BIC have different punishment for more lags. Here the punishment coefficient of BIC is much bigger($log(6863) > 2$), thus less lags are chosen in BIC optimal model.
